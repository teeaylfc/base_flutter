import 'package:oev_timesheet/src/common/util/timeago/vn_messages_timeago.dart';

import 'en_messages_timeago.dart';
...
String format(DateTime date, String hhmm, String dateString,
  ...
  String prefix, suffix, suffixAgoDay, now;
  ...
  if (_allowFromNow && elapsed < 0) {
    ...
    suffixAgoDay = messages.suffixAgoDay();
  }
  now = messages.suffixFromNow();
  ...
  if (seconds < 45)
    ...
    result = messages.aboutAMinute(minutes.round()) + ' ' + suffix;
  else if (minutes < 45)
    result = messages.minutes(minutes.round()) + ' ' + suffix;
  else if (minutes < 90)
    result = messages.aboutAnHour(minutes.round()) + ' ' + suffix;
  else if (hours < 24)
    result = messages.hours(hours.round()) + ' ' + suffix;
  else if (hours < 48)
    result = messages.aDay(hours.round()) + ' ' + hhmm;
  else if (days < 6)
    result = messages.days(days.round()) + ' ' + suffix;
  else
  ...
  // else if (days < 365)
  //   result =
  //       messages.months(months.round()) + ' ' + suffixAgoDay + ' ' + dateString;
  // else if (years < 2)
